...
import { PullRequestReferencePill, ReferencePill } from "./ReferencePill";
  ...
  const refresh = useCallback(async (showLoading = false) => {
    ...
    try {
      ...
      if (data.length === 0 && _issueCache && _issueCache.length > 0) {
        console.log(
          "[MyIssues] Ignoring empty response; keeping cached",
          _issueCache.length,
          "issues",
        );
      } else {
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex items-center justify-center gap-2 py-12 text-sm text-muted-foreground">
            <CheckCircle className="h-4 w-4" aria-hidden="true" /> No open
            issues
          </div>
        ) : (
          <ul className="divide-y divide-border">
            {issues.map((issue) => {
              ...
              const showCopilotBtn = !isDemo && !hasOpenPR && !copilotAssigned;
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5">
                        ...
                        {copilotAssigned && (
                          <span
                            className="inline-flex items-center gap-1 rounded-full border border-border bg-muted px-1.5 py-0.5 text-[10px] font-medium text-foreground"
