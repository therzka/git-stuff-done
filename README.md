...
- **🏃 WalkingPad** — Control a KingSmith WalkingPad C2 treadmill via Web Bluetooth (Chrome/Edge). Connect, start/stop, adjust speed with large +/- buttons (0.5–3.75 mph, 0.25 increments, default 0.8 mph), and view live stats (speed, distance, time, steps, calories). Speed controls use direct per-click commands, show protocol/mode context, and disable in standard auto mode (with guidance to switch to manual mode). Walk sessions auto-log to `data/walks.json` when the treadmill stops. Includes session history (no insert button) and daily/weekly distance charts. Hidden by default — enable from the ☰ panel menu. Uses [`walkingpad-js`](https://github.com/Mwea/walkingpad-js).
