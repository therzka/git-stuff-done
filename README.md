...
- **▤ Layout modes** — Switch between grid (2-column), column (single-column), and row (single horizontal strip for widescreen setups) layouts. Hide individual panels and restore them from the ☰ menu. **Drag any panel by its card surface to reorder it or move it between grid columns.** Positions, order, and visibility are all saved in localStorage.
